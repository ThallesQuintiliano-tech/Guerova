...
const SubMenu = ({ item, toggleMobileSidebar }) => {
  ...
  const campaignFlow = /^\/leadmaster\/campanha/.test(path);

  // Determine if the parent or any child is active
  const isActive =
    path === item.to ||
    (item.to === '/leadmaster/campanhas' && campaignFlow) ||
    (hasSubmenu && item.content.some((child) => child.to === path));
  ...
  return (
    <Fragment>
      <div className="vertical-nav-menu">
        <h5 className="app-sidebar__heading">LeadMaster AI</h5>
        <ul className="metismenu-container">{renderMenu(LeadMasterNav)}</ul>
        ...
        {isLeadMaster && (
          <>
            <h5 className="app-sidebar__heading">Tema</h5>
            <ul className="metismenu-container">{renderMenu(ArchitectUiTemplateNav)}</ul>
          </>
        )}
      </div>
    </Fragment>
